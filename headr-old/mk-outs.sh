...
for FILE in $INPUTS/*.txt; do
    ...
    head -n -3 $FILE > ${OUT_DIR}/${BASENAME}.n-3.out
    head -c 1K $FILE > ${OUT_DIR}/${BASENAME}.c1k.out
    head -c 2M $FILE > ${OUT_DIR}/${BASENAME}.c2m.out
done
...
head -n -3 $ALL > $OUT_DIR/all.n-3.out
head -c 1K $ALL > $OUT_DIR/all.c1k.out
head -c 2M $ALL > $OUT_DIR/all.c2m.out
